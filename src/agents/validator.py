...
class Validator:
    ...
    def analyze_unmatched_results(
        ...
        2. Date proximity within the configured date window.
        3. Total proximity within the configured amount threshold.
        ...
        def build_user_facing_reason(
            ...
            if len(labels) == 1:
                return f"Similar {labels[0]}"
            return f"Similar {', '.join(labels[:-1])} and {labels[-1]}"
